...
@protocol FPSegmentedControlDelegate;

@interface FPSegmentedControl : UIControl

@property (nonatomic, copy) void (^changeHandler)(NSUInteger newIndex); // you can also use addTarget:action:forControlEvents:

@property (nonatomic, strong, readonly) FPSegmentedThumb *thumb;
@property (nonatomic, strong, readonly) NSMutableArray *titlesArray;
...
@property (nonatomic, unsafe_unretained) id<FPSegmentedControlDelegate> delegate DEPRECATED_ATTRIBUTE; // use addTarget:action:forControlEvents: instead
@property (nonatomic, readwrite) CGFloat segmentPadding DEPRECATED_ATTRIBUTE; // use titleEdgeInsets instead

- (FPSegmentedControl*)initWithSectionTitles:(NSArray*)titlesArray;
...
- (void)segmentedControl:(FPSegmentedControl*)segmentedControl didSelectIndex:(NSUInteger)index;
