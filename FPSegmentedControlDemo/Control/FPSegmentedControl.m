...
#define FPSegmentedControlBG [[UIImage imageNamed:@"SVSegmentedControl.bundle/inner-shadow"] stretchableImageWithLeftCapWidth:4 topCapHeight:5]

@interface FPSegmentedThumb ()

@property (nonatomic, assign) FPSegmentedControl *segmentedControl;
...
@property (nonatomic, readwrite) UIEdgeInsets thumbEdgeInset; // constant = UIEdgeInsetsZero

@end


@implementation FPSegmentedControl
...
- (id)initWithSectionTitles:(NSArray*)array {
	...
	if (self = [super initWithFrame:CGRectZero]) {
        ...
        self.thumbEdgeInset = UIEdgeInsetsZero;
        self.height = 32.0;
...
- (FPSegmentedThumb *)thumb {
    
    if(thumb == nil)
        thumb = [[FPSegmentedThumb alloc] initWithFrame:CGRectZero];
...
- (void)drawRect:(CGRect)rect {
	...
	for(NSString *titleString in self.titlesArray) {
        ...
        if (i) {
            ...
            [linePath moveToPoint:CGPointMake(labelRect.origin.x, rect.origin.y)];
            [linePath addLineToPoint:CGPointMake(labelRect.origin.x, rect.origin.y + rect.size.height)];
